...
export default function AlertsPage() {
  const { data, loading, error, refresh } = useApi<AlertsResponse>("/api/alerts?evaluate=true");
  ...
  async function create() {
    setStatus(null);
    try {
      await postJson("/api/alerts", { symbol, kind, threshold, direction: selected.direction ? direction : null, note });
  ...
  async function toggle(alert: Alert) {
    await postJson(`/api/alerts/${alert.id}`, { active: !alert.active }, "PATCH");
    refresh();
  }
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <Panel className="p-5">
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          {status ? <span className="text-xs text-gold">{status}</span> : null}
        </div>
      </Panel>
      ...
      <p className="text-xs text-faint">
        Alerts are evaluated whenever this page loads. Browser, email and push delivery are configured in Settings; SMS can be added later
        through the same notification abstraction.
      </p>
    </div>
  );
}
